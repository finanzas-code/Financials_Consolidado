...
const CATEGORY_RULES = [
  ...
  {
    category: "professional_fees_interco",
    type: "expense",
    prefixes: ["6230100069"],
    keywords: [],
  },
  ...
  },
  {
    ...
    type: "income",
    prefixes: ["7520"],
    keywords: ["alquiler", "recurrente alquiler", "renta"],
  },
  {
    category: "rnt_sales",
    type: "income",
  ...
  },
  {
    ...
    keywords: ["subvencion", "ingreso excepcional"],
  },
  {
    category: "rental_costs",
    type: "expense",
    prefixes: ["6280", "6281", "6282"],
    keywords: [
      "suministro",
      "luz",
      "agua",
  ...
  },
  {
    ...
    keywords: ["comunidad", "limpieza", "mantenimiento"],
  },
  {
  ...
  },
  {
    ...
    type: "expense",
    prefixes: ["6290040"],
    keywords: ["plataformas operativas", "google cloud", "webflow", "saas"],
  },
  {
    category: "ga_personnel",
    type: "expense",
    prefixes: ["6220", "6230", "6420", "6400", "6410"],
    keywords: [
      "nomina",
      "personal",
      "salario",
  ...
  },
  {
    ...
    type: "expense",
    prefixes: ["6210", "6250", "6260", "6280000", "6290000"],
    keywords: [
      "seguro",
      "bancario",
      "comision banco",
  ...
  },
  {
    ...
    type: "mixed",
    prefixes: ["7680", "6680"],
    keywords: ["diferencia cambio", "divisa"],
  },
  {
    category: "bank_interests",
    type: "mixed",
    prefixes: ["7620", "7690", "6623"],
    keywords: ["interes banco", "rendimiento", "ingresos financieros"],
  },
  {
    category: "other_expenses",
    type: "expense",
    prefixes: ["6620", "6310", "6780"],
    keywords: ["ibi", "impuesto", "itp", "tributo", "no deducible"],
  },
];
const QB_PL_MAP = {
...
function exportToExcel(consData, consIS, empresasData, year, fxRates, fxAvg) {
  ...
  const n = (v, color, bg, bold = false, fmt = numFmt) => ({
    v: v || 0, t: "n", z: fmt,
    s: {
  ...
  const nPct = (v, bg) => ({ v: (v || 0) / 100, t: "n", z: pctFmt,
    s: { font: { name: "Calibri", sz: 10, color: { rgb: C.slate2 } },
         fill: { fgColor: { rgb: bg } }, alignment: { horizontal: "right" } } });

  // número coloreado positivo=verde / negativo=rojo
  const nColor = (v, bg, bold = false) =>
    n(v, v >= 0 ? C.green : C.red, bg, bold);

  // número gris (meses sin dato)
  const nGray = (v, bg) => n(v, v !== 0 ? C.slate : C.zero, bg);

  // ── Estilos de fila ───────────────────────────────────────────────────────
  const HDR = { font: { name: "Calibri", sz: 10, bold: true, color: { rgb: C.white } },
                fill: { fgColor: { rgb: C.navy } }, alignment: { horizontal: "center" } };
  const HDR_R = { ...HDR, alignment: { horizontal: "right" } };

  const rowHdrStyle = (color) => ({
    font: { name: "Calibri", sz: 10, bold: true, color: { rgb: color } },
    fill: { fgColor: { rgb: C.navyBg } },
  });
  const rowSubStyle = {
    font: { name: "Calibri", sz: 10, color: { rgb: C.slate } },
    fill: { fgColor: { rgb: C.rowLight } },
    alignment: { indent: 1 },
  };
  const rowNetRevStyle = {
    font: { name: "Calibri", sz: 10, bold: true, color: { rgb: C.blue } },
    fill: { fgColor: { rgb: C.rowLight } },
  };
  const rowDedStyle = {
    font: { name: "Calibri", sz: 10, bold: true, color: { rgb: C.slate } },
    fill: { fgColor: { rgb: C.rowAlt } },
  };
  const rowOpexStyle = {
    font: { name: "Calibri", sz: 10, bold: true, color: { rgb: C.slate } },
    fill: { fgColor: { rgb: C.rowOpex } },
  };
  const rowEbStyle = {
    font: { name: "Calibri", sz: 11, bold: true, color: { rgb: C.green } },
    fill: { fgColor: { rgb: C.rowEb } },
  };
  const rowNetStyle = {
    font: { name: "Calibri", sz: 11, bold: true, color: { rgb: C.navy } },
    fill: { fgColor: { rgb: C.navyBg2 } },
  };

  // ── Función que construye una hoja P&G (consolidado o empresa) ─────────────
  function buildPYGSheet(data, is, currency, isUSD, emp = null) {
    const aoa = [];
    const rate = isUSD ? 1 : fxAvg;
    const otherCurrencyLabel = isUSD ? "EUR equiv." : "USD equiv.";

    // Títulos
    const sheetTitle = emp
      ? `${emp.name.toUpperCase()}  ·  P&G ${year}  (${currency})`
      : `P&G CONSOLIDADO  ·  GRUPO REENTAL ${year}  ·  USD`;
    aoa.push([s(sheetTitle, { font: { name: "Calibri", sz: 13, bold: true, color: { rgb: C.navy } } })]);

    const subtitleText = emp
      ? `Moneda nativa ${currency}  ·  USD a avg BCE ${fxAvg.toFixed(4)}  ·  ${today}`
      : `5 entidades · Interco eliminadas · BCE avg (${knownMonths}m) = ${fxAvg.toFixed(4)} EUR/USD · ${today}`;
    aoa.push([s(subtitleText, { font: { name: "Calibri", sz: 10, color: { rgb: C.grayText } } })]);

    // Aviso interco si aplica
    if (emp && CONS_EXCLUSIONS[emp.name]?.length) {
      const excl = CONS_EXCLUSIONS[emp.name].join(", ");
      aoa.push([s(
        `⚠ ${excl} excluidos del consolidado (interco)`,
        { font: { name: "Calibri", sz: 9, color: { rgb: C.amber } }, fill: { fgColor: { rgb: C.amberBg } } }
      )]);
    } else {
      aoa.push([]);
    }
    ...
    aoa.push([
      s("Línea P&G", HDR), s("", HDR), s("", HDR),
      s(`TOTAL ${currency}`, HDR_R),
      s(otherCurrencyLabel, HDR_R),
      s("% Ing.", HDR_R),
      ...meses.map(mo => s(mo, HDR_R)),
    ]);

    // Fila tipo de cambio (solo consolidado)
    if (!emp) {
      aoa.push([
        s("Tipo EUR/USD (BCE)", { font: { name: "Calibri", sz: 10, color: { rgb: C.grayText } }, fill: { fgColor: { rgb: C.rowAlt } } }),
        s("", { fill: { fgColor: { rgb: C.rowAlt } } }),
        s("", { fill: { fgColor: { rgb: C.rowAlt } } }),
        n(fxAvg, C.navy, C.rowAlt, true, "0.0000"),
        s("", { fill: { fgColor: { rgb: C.rowAlt } } }),
        s("", { fill: { fgColor: { rgb: C.rowAlt } } }),
        ...meses.map((_, i) => {
          const m = i + 1;
          return n(fxRates[m] || fxAvg, fxRates[m] ? C.navy : C.grayLt, C.rowAlt, !!fxRates[m], "0.0000");
        }),
      ]);
    }
    ...
    const t = (k) => Object.values(data).reduce((s, m) => s + (m[k] || 0), 0);
    const mv = (fn) => meses.map((_, i) => fn(data[i + 1] || {}));

    // Utilidades de conversión
    const toOther = (v) => isUSD ? v / fxAvg : v * fxAvg;

    // ── GROSS REVENUES ──────────────────────────────────────────────────────
    const gross = is.gross;
    aoa.push([
      s("GROSS REVENUES", rowHdrStyle(C.navy)), s("", { fill: { fgColor: { rgb: C.navyBg } } }), s("", { fill: { fgColor: { rgb: C.navyBg } } }),
      n(gross, C.navy, C.navyBg, true),
      n(toOther(gross), C.grayText, C.navyBg),
      s("", { fill: { fgColor: { rgb: C.navyBg } } }),
      ...mv((d) => {
        const v = (d.setup_fees||0)+(d.rental_contracts||0)+(d.rnt_sales||0)+(d.properties_sale||0)+(d.other_revenues||0);
        return nColor(v, C.navyBg, true);
      }),
    ]);

    [
      ["  Set up fees", "setup_fees"],
      ...
      const v = t(cat);
      aoa.push([
        s(lbl, rowSubStyle), s("", { fill: { fgColor: { rgb: C.rowLight } } }), s("", { fill: { fgColor: { rgb: C.rowLight } } }),
        n(v, C.slate2, C.rowLight),
        n(toOther(v), C.slate2, C.rowLight),
        nPct(gross > 0 ? (v / gross) * 100 : 0, C.rowLight),
        ...mv((d) => nGray(d[cat] || 0, C.rowLight)),
      ]);
    });
    ...
    // ── DEDUCTIONS ──────────────────────────────────────────────────────────
    const ded = is.deductions;
    aoa.push([
      s("DEDUCTIONS & DIRECT COSTS", rowDedStyle), s("", { fill: { fgColor: { rgb: C.rowAlt } } }), s("", { fill: { fgColor: { rgb: C.rowAlt } } }),
      n(-ded, C.slate, C.rowAlt, true),
      n(toOther(-ded), C.grayText, C.rowAlt),
      s("", { fill: { fgColor: { rgb: C.rowAlt } } }),
      ...mv((d) => {
        const v = -((d.rental_costs||0)+(d.rnt_expenses||0)+(d.properties_sale_cost||0));
        return n(v, v < 0 ? C.red : C.grayText, C.rowAlt, true);
      }),
    ]);
    ...
    [
      ...
      aoa.push([
        ...
        n(-v, C.slate2, C.rowLight),
        n(toOther(-v), C.slate2, C.rowLight),
        s("", { fill: { fgColor: { rgb: C.rowLight } } }),
        ...mv((d) => {
          const mv2 = -(d[cat] || 0);
          return n(mv2, mv2 < 0 ? C.red : C.grayText, C.rowLight);
        }),
      ]);
    });
    ...
    // ── NET REVENUES ────────────────────────────────────────────────────────
    const netRev = is.net_rev;
    aoa.push([
      s("NET REVENUES", rowNetRevStyle), s("", { fill: { fgColor: { rgb: C.rowLight } } }), s("", { fill: { fgColor: { rgb: C.rowLight } } }),
      n(netRev, C.blue, C.rowLight, true),
      n(toOther(netRev), C.grayText, C.rowLight),
      nPct(is.gross_margin, C.rowLight),
      ...mv((d) => {
        const g = (d.setup_fees||0)+(d.rental_contracts||0)+(d.rnt_sales||0)+(d.properties_sale||0)+(d.other_revenues||0);
        const v = g - (d.rental_costs||0) - (d.rnt_expenses||0) - (d.properties_sale_cost||0);
        return nColor(v, C.rowLight, true);
      }),
    ]);
    ...
    const opex = is.opex;
    aoa.push([
      s("TOTAL OPEX", rowOpexStyle), s("", { fill: { fgColor: { rgb: C.rowOpex } } }), s("", { fill: { fgColor: { rgb: C.rowOpex } } }),
      n(-opex, C.slate, C.rowOpex, true),
      n(toOther(-opex), C.grayText, C.rowOpex),
      nPct(gross > 0 ? (opex / gross) * 100 : 0, C.rowOpex),
      ...mv((d) => {
        const v = -((d.tech_costs||0)+(d.ga_personnel||0)+(d.ga_suppliers||0)+(d.sales_marketing||0));
        return n(v, v < 0 ? C.red : C.grayText, C.rowOpex, true);
      }),
    ]);
    ...
    aoa.push([
      s("EBITDA", rowEbStyle), s("", { fill: { fgColor: { rgb: C.rowEb } } }), s("", { fill: { fgColor: { rgb: C.rowEb } } }),
      nColor(ebitda, C.rowEb, true),
      n(toOther(ebitda), C.grayText, C.rowEb),
      nPct(is.ebitda_margin, C.rowEb),
      ...mv((d) => {
        const g = (d.setup_fees||0)+(d.rental_contracts||0)+(d.rnt_sales||0)+(d.properties_sale||0)+(d.other_revenues||0);
        const ded2 = (d.rental_costs||0)+(d.rnt_expenses||0)+(d.properties_sale_cost||0);
        const opex2 = (d.tech_costs||0)+(d.ga_personnel||0)+(d.ga_suppliers||0)+(d.sales_marketing||0);
        return nColor(g - ded2 - opex2, C.rowEb, true);
      }),
    ]);
    ...
    // ── BELOW EBITDA ────────────────────────────────────────────────────────
    [
      ["  Interests token holders", "interests_token_holders"],
      ...
      aoa.push([
        ...
        ...mv((d) => {
          const mv2 = d[cat] || 0;
          return n(mv2, mv2 < 0 ? C.red : mv2 > 0 ? C.slate : C.grayText, C.rowLight);
        }),
      ]);
    });
    ...
    // ── NET PROFIT ──────────────────────────────────────────────────────────
    const netProfit = is.net_profit;
    aoa.push([
      s("NET PROFIT / LOSS", rowNetStyle), s("", { fill: { fgColor: { rgb: C.navyBg2 } } }), s("", { fill: { fgColor: { rgb: C.navyBg2 } } }),
      nColor(netProfit, C.navyBg2, true),
      n(toOther(netProfit), C.grayText, C.navyBg2),
      nPct(is.net_margin, C.navyBg2),
      ...mv((d) => {
        const g = (d.setup_fees||0)+(d.rental_contracts||0)+(d.rnt_sales||0)+(d.properties_sale||0)+(d.other_revenues||0);
        const ded2 = (d.rental_costs||0)+(d.rnt_expenses||0)+(d.properties_sale_cost||0);
        const opex2 = (d.tech_costs||0)+(d.ga_personnel||0)+(d.ga_suppliers||0)+(d.sales_marketing||0);
        const fin2 = (d.interests_token_holders||0)+(d.fx_gain_loss||0)+(d.bank_interests||0)+(d.other_expenses||0);
        return nColor(g - ded2 - opex2 - fin2, C.navyBg2, true);
      }),
    ]);

    const ws = XLSX.utils.aoa_to_sheet(aoa);
    const extraCols = emp ? 0 : 1; // consolidado tiene fila tipo cambio → no afecta cols
    ws["!cols"] = [
      { wch: 30 }, { wch: 2 }, { wch: 2 },
      { wch: 16 }, { wch: 14 }, { wch: 8 },
      ...meses.map(() => ({ wch: 13 })),
  ...
  {
    const aoa = [];
    aoa.push([s(`GRUPO REENTAL  ·  P&G CONSOLIDADO ${year}`, {
      font: { name: "Calibri", sz: 14, bold: true, color: { rgb: C.navy } },
    })]);
    aoa.push([s(`Informe financiero consolidado  ·  5 entidades  ·  Moneda: USD  ·  ${today}`, {
      font: { name: "Calibri", sz: 10, color: { rgb: C.grayText } },
    })]);
    aoa.push([]);
    aoa.push([s(
      `Tipos BCE/BdE  ·  ${knownMonths} meses reales  ·  Media parcial: ${fxAvg.toFixed(4)} EUR/USD  ·  Interco eliminadas: setup_fees (SLs) + ga_personnel (America LLC)`,
      { font: { name: "Calibri", sz: 9, color: { rgb: C.amber } }, fill: { fgColor: { rgb: C.amberBg } } }
    )]);
    aoa.push([]);

    // KPIs consolidado
    const kpiHdrStyle = { font: { name: "Calibri", sz: 10, bold: true, color: { rgb: C.white } }, fill: { fgColor: { rgb: C.navy } } };
    aoa.push([
      s("INDICADOR CLAVE", kpiHdrStyle), s("USD (total año)", kpiHdrStyle),
      s("EUR equiv.", kpiHdrStyle), s("% s/Ingresos", kpiHdrStyle), s("Descripción", kpiHdrStyle),
    ]);

    const kpiRows = [
      { lbl: "Gross Revenues", val: consIS.gross, desc: "Ingresos brutos totales consolidados", bg: C.navyBg },
      { lbl: "Deductions", val: -consIS.deductions, desc: "Costes directos y bajas de activos", bg: C.rowAlt },
      { lbl: "Net Revenues", val: consIS.net_rev, pct: consIS.gross_margin, desc: `Margen bruto ${consIS.gross_margin.toFixed(1)}%`, bg: C.rowLight },
      { lbl: "Total OPEX", val: -consIS.opex, pct: consIS.gross > 0 ? (consIS.opex/consIS.gross)*100 : 0, desc: "G&A + Tech + Sales & Marketing", bg: C.rowAlt },
      { lbl: "EBITDA", val: consIS.ebitda, pct: consIS.ebitda_margin, desc: "Resultado operativo recurrente", bg: C.rowEb, bold: true },
      { lbl: "Resultados financieros", val: -consIS.fin, desc: "TH interests · FX · banco · otros", bg: C.rowAlt },
      { lbl: "Net Profit / Loss", val: consIS.net_profit, pct: consIS.net_margin, desc: "Resultado neto del ejercicio", bg: C.navyBg2, bold: true },
    ];

    kpiRows.forEach(({ lbl, val, pct: pctVal, desc, bg, bold: isB = false }) => {
      const lColor = isB ? (lbl === "EBITDA" ? C.green : C.navy) : C.slate;
      aoa.push([
        s(lbl, { font: { name: "Calibri", sz: 10, bold: isB, color: { rgb: lColor } }, fill: { fgColor: { rgb: bg } } }),
        nColor(val, bg, isB),
        n(val / fxAvg, C.grayText, bg),
        pctVal !== undefined ? nPct(pctVal, bg) : s("", { fill: { fgColor: { rgb: bg } } }),
        s(desc, { font: { name: "Calibri", sz: 10, color: { rgb: C.grayText } }, fill: { fgColor: { rgb: bg } } }),
      ]);
    });
    ...
    // Tabla por entidad
    const entHdrStyle = { font: { name: "Calibri", sz: 10, bold: true, color: { rgb: C.white } }, fill: { fgColor: { rgb: C.navy } } };
    aoa.push([
      s("ENTIDAD", entHdrStyle), s("DIVISA", entHdrStyle),
      s("Gross USD", entHdrStyle), s("EBITDA USD", entHdrStyle),
      s("Net Profit USD", entHdrStyle), s("Mg. EBITDA", entHdrStyle), s("Mg. Neto", entHdrStyle),
    ]);

    EMPRESAS_CONFIG.forEach((emp, idx) => {
      const empData = empresasData[emp.name] || {};
      const eis = calcIS(empData);
      const toUSD = emp.currency === "USD" ? 1 : fxAvg;
      const rowBg = idx % 2 === 0 ? "F7F8FA" : C.white;
      aoa.push([
        s(emp.name, { font: { name: "Calibri", sz: 10, bold: true, color: { rgb: C.darkText } }, fill: { fgColor: { rgb: rowBg } } }),
        s(emp.currency, { font: { name: "Calibri", sz: 10, color: { rgb: C.grayText } }, fill: { fgColor: { rgb: rowBg } } }),
        nColor(eis.gross * toUSD, rowBg),
        nColor(eis.ebitda * toUSD, rowBg),
        nColor(eis.net_profit * toUSD, rowBg),
        nPct(eis.ebitda_margin, rowBg),
        nPct(eis.net_margin, rowBg),
      ]);
    });

    // Fila consolidado
    aoa.push([
      s("CONSOLIDADO  (interco eliminadas)", { font: { name: "Calibri", sz: 10, bold: true, color: { rgb: C.navy } }, fill: { fgColor: { rgb: C.navyBg2 } } }),
      s("USD", { font: { name: "Calibri", sz: 10, color: { rgb: C.slate2 } }, fill: { fgColor: { rgb: C.navyBg2 } } }),
      nColor(consIS.gross, C.navyBg2, true),
      nColor(consIS.ebitda, C.navyBg2, true),
      nColor(consIS.net_profit, C.navyBg2, true),
      nPct(consIS.ebitda_margin, C.navyBg2),
      nPct(consIS.net_margin, C.navyBg2),
    ]);

    aoa.push([]);
    aoa.push([s(
      `Fuente: BCE EXR.M.USD.EUR.SP00.A  ·  ${knownMonths} meses reales  ·  Media parcial ${fxAvg.toFixed(4)} aplicada a meses pendientes`,
      { font: { name: "Calibri", sz: 9, color: { rgb: C.grayLt } } }
    )]);

    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws["!cols"] = [{ wch: 34 }, { wch: 10 }, { wch: 18 }, { wch: 18 }, { wch: 18 }, { wch: 12 }, { wch: 12 }];
    XLSX.utils.book_append_sheet(wb, ws, "Resumen Ejecutivo");
  }

  // ── 2. P&G Consolidado USD ─────────────────────────────────────────────────
  {
    const ws = buildPYGSheet(consData, consIS, "USD", true);
    XLSX.utils.book_append_sheet(wb, ws, "P&G Consolidado USD");
  }

  // ── 3. Hojas individuales ──────────────────────────────────────────────────
  const sheetNames = {
    "Rentaltoken SL": "Rentaltoken",
    "Admin RNT SL": "Admin RNT",
    "Reental Tourist Homes": "Tourist Homes",
    "Reental Rock Capital": "Rock Capital",
    "Reental America LLC": "America",
  };

  EMPRESAS_CONFIG.forEach((emp) => {
    const empData = empresasData[emp.name] || {};
    const eis = calcIS(empData);
    const isUSD = emp.currency === "USD";
    const ws = buildPYGSheet(empData, eis, emp.currency, isUSD, emp);
    XLSX.utils.book_append_sheet(wb, ws, sheetNames[emp.name] || emp.name.slice(0, 20));
  });

  // ── 4. Hoja Tipos BCE ──────────────────────────────────────────────────────
  {
    const aoa = [];
    aoa.push([s(`TIPOS EUR/USD BCE  ·  ${year}`, { font: { name: "Calibri", sz: 13, bold: true, color: { rgb: C.navy } } })]);
    aoa.push([s(`Serie: EXR.M.USD.EUR.SP00.A  ·  ${knownMonths} meses publicados  ·  ${today}`, { font: { name: "Calibri", sz: 10, color: { rgb: C.grayText } } })]);
    aoa.push([]);
    aoa.push([s("Mes", HDR), s("EUR/USD (media mensual)", HDR_R), s("Estado", HDR), s("Desv. vs media", HDR_R)]);

    for (let m = 1; m <= 12; m++) {
      const real = !!fxRates[m];
      const rate2 = fxRates[m] || fxAvg;
      const bg = real ? C.navyBg : "F7F8FA";
      const deviation = rate2 - fxAvg;
      aoa.push([
        s(MONTHS[m], { font: { name: "Calibri", sz: 10, bold: real, color: { rgb: real ? C.darkText : C.grayText } }, fill: { fgColor: { rgb: bg } } }),
        n(rate2, real ? C.navy : C.grayText, bg, real, "0.0000"),
        s(real ? "✓  BCE real" : "~  avg parcial", {
          font: { name: "Calibri", sz: 10, color: { rgb: real ? C.blue : C.grayLt } },
          fill: { fgColor: { rgb: bg } },
        }),
        real ? n(deviation, deviation >= 0 ? C.green : C.red, bg, false, "+0.0000;-0.0000;-") : s("", { fill: { fgColor: { rgb: bg } } }),
      ]);
    }

    aoa.push([]);
    aoa.push([
      s("Media parcial  (2m reales)", { font: { name: "Calibri", sz: 10, bold: true, color: { rgb: C.navy } }, fill: { fgColor: { rgb: C.navyBg2 } } }),
      n(fxAvg, C.navy, C.navyBg2, true, "0.0000"),
      s("BCE oficial", { font: { name: "Calibri", sz: 10, color: { rgb: C.slate2 } }, fill: { fgColor: { rgb: C.navyBg2 } } }),
    ]);

    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws["!cols"] = [{ wch: 12 }, { wch: 22 }, { wch: 16 }, { wch: 16 }];
    XLSX.utils.book_append_sheet(wb, ws, "Tipos BCE");
  }

  XLSX.writeFile(wb, `PYG_GrupoReental_${year}_${new Date().toISOString().slice(0, 10)}.xlsx`);
}
...
export default function App() {
  ...
  // ─── INICIALIZAR ESTADO DESDE HASH ────────────────────────────────────────
  const savedState = loadStateFromHash();

  const [empresasData, setEmpresasData] = useState(
    savedState
      ? Object.fromEntries(
          EMPRESAS_CONFIG.map((e) => [e.name, savedState[e.name] || emptyMonths()])
        )
  ...
  const [loadedFiles, setLoadedFiles] = useState(
    savedState
      ? Object.fromEntries(Object.keys(savedState).map((k) => [k, true]))
      : {}
  );

  // ─── GUARDAR ESTADO EN HASH CUANDO CAMBIAN LOS DATOS ─────────────────────
  useEffect(() => {
    ...
    try {
      const payload = loaded.reduce((acc, name) => {
        acc[name] = empresasData[name];
        return acc;
      }, {});
      window.location.hash = btoa(unescape(encodeURIComponent(JSON.stringify(payload))));
    } catch (e) {
      console.warn("No se pudo serializar el estado:", e);
    }
  }, [empresasData, loadedFiles]);
